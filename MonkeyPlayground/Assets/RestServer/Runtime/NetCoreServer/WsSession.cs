...
namespace RestServer.Runtime.NetCoreServer {
    ...
    public class WsSession : HttpSession, IWebSocket
    {
        ...
        protected void DoOnReceivedRequestHeaderWithUpgrade(HttpRequest request, bool allowUpgrade) {
            // Check for WebSocket handshaked status
            if (WebSocket.WsHandshaked)
                return;

            // Try to perform WebSocket upgrade
            if (allowUpgrade && !WebSocket.PerformServerUpgrade(request, Response))
            {
