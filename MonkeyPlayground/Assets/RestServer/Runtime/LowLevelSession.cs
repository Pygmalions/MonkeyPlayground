...
using RestServer.Runtime.NetCoreServer;
using UnityEngine;
    ...
    public class LowLevelSession : WsSession {
        ...
        protected override void OnReceivedRequest(HttpRequest request) {
            try {
                ...
                // Disconnect(); - can't disconnect here as the async buffer still needs to empty (?)
            }
        }
        ...
        protected virtual void OnReceivedRequestExecuteHandler(HttpRequest request, LowLevelServer server, Uri requestUri, RestRequest requestAndResponse) {
            ...
            while (true) {
                ...
                if (endpoint.HasValue) {
                    ...
                    if (endpoint.Value.WebSocketUpgradeAllowed) {
                        // websocket upgrade was allowed, but we are in the HTTP path --> no ws request or failed upgrade
        ...
        public override void OnWsReceived(byte[] buffer, long offset, long size) {
            ...
            if (_endpoint.HasValue) {
                try {
                    // cut the buffer according to the offset+size
                    var temp = buffer.AsSpan((int) offset, (int) size).ToArray();
                    
                    _endpoint.Value.WebSocketFrameHandler(new WebsocketMessage(temp, this));
                }
                catch (Exception e) {
        ...
        public override void OnWsClose(byte[] buffer, long offset, long size, int status = 1000) {
            ...
            base.OnWsClose(buffer, offset, size, status); // this closes the endpoint (!)
        }
