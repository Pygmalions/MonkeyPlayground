...
namespace RestServer.Runtime {
    ...
    public class ThreadingHelper {
        ...
        public bool IsExecuteSyncActive { get; private set; }
        ...
        public void EnqueueWorkload(Workload workload) {
            if (object.ReferenceEquals(Thread.CurrentThread, _mainThreadReference)) {
                // Enqueuing workloads from the main thread does result in a deadlock (for ExecuteSync).
                // Since you are already on the main thread, you can just execute the unity code
        ...
        /// <summary>
        /// Throws an exception if not inside the rest request or inside ExecuteSync. 
        /// </summary>
        /// <exception cref="SystemException"></exception>
        public void ThrowIfNotAllowedToSendResponse() {
            if (IsExecuteSyncActive) { // assume that it's this request. Not 100% foolproof but should be enough
                return;
            }

            if (Thread.CurrentThread.ManagedThreadId == _mainThreadReference.ManagedThreadId) {
                throw new SystemException(
                    "The SendAsync method can't be called from a different thread. " +
                    "Did you accidentally call it from ExecuteAsync? Use ExecuteSync instead."
                );
            }
        }
    }
