...
namespace de.bearo.restserver.Samples.WebSocketExample {
    ...
    public class WSMover : MonoBehaviour {
        ...
        public void Start() {
            Server.AdvancedCustomization = new MyAdvancedCustomization();
        ...
        private void OnGUI() {
            ...
            var now = DateTime.Now;
            GUI.Label(new Rect(10, 10, 200, 20), now.ToString("HH:mm:ss:fff"));
        }
    ...
    internal class MyAdvancedCustomization : AdvancedCustomization {
        public override void OnWsConnect(WSEndpointId endpointId, LowLevelSession session) {
            Debug.Log("New connection: " + endpointId.Id + " Address: " + session.Socket.RemoteEndPoint);
        }
        
        public override void OnWsDisconnect(WSEndpointId endpointId, LowLevelSession session) {
            Debug.Log("Disconnect: " + endpointId.Id + " Address: " + session.Socket.RemoteEndPoint);
        }
    }
}